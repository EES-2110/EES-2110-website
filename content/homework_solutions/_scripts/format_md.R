...
format_md = function(x, digits = NULL,
                     format = c('normal', 'auto', 'scientific',
                                'engineering'),
                     comma = FALSE,
                     output_format = c("markdown", "latex", "bare-latex")) {
  format = match.arg(format)
  output_format = match.arg(output_format)
  mark = ifelse(comma, ',', '')

  fixup_scientific = function(s, output_format = c("markdown", "latex", "bare-latex")) {
    output_format = match.arg(output_format)
    if (output_format == 'markdown') {
    str_replace_all(s,  c('\\+' = '',
                          '[Ee](-?)0*([1-9][0-9]*)$' = '&times;10^\\1\\2^'))
    } else {
      res <- str_replace_all(s,  c('\\+' = '',
  ...
  if (format == 'auto') {
    ...
    formatted = ifelse(str_length(fixed) > getOption('scipen') + str_length(sci),
                      fixup_scientific(sci, output_format = output_format), fixed)
